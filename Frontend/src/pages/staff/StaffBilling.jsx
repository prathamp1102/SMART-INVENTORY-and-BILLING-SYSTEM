...
/* ─── Org Logo Badge (matches sidebar EVARA style) ───────── */
function OrgLogoBadge({ name }) {
  const letter = (name || "O").charAt(0).toUpperCase();
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "11px" }}>
      {/* Green square icon — mirrors the sidebar logo */}
      <div style={{
        width: 44, height: 44,
        borderRadius: "12px",
        ...
        flexShrink: 0,
        boxShadow: "0 4px 14px rgba(5,150,105,.38)",
      }}>
        <span style={{
          fontFamily: ff.serif,
          fontSize: "21px",
    ...
    <div style={{ animation: "fadeUp .35s ease both" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div>
            <div style={{ fontFamily: ff.serif, fontSize: "20px", fontWeight: 800, color: C.ink }}>Invoice Generated</div>
            <div style={{ fontSize: "12px", color: C.inkA(.4), fontFamily: ff.mono, marginTop: "1px" }}>#{invoiceNumber}</div>
          </div>
        </div>
      ...
      <Card style={{ maxWidth: "680px", padding: "36px 40px" }}>
        <div ref={printRef}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "28px", paddingBottom: "24px", borderBottom: `2px solid ${C.inkA(.07)}` }}>
            <OrgLogoBadge name={orgName || "Organisation"} />

            <div style={{ textAlign: "right" }}>
              <div style={{ fontFamily: ff.mono, fontSize: "14px", fontWeight: 700, color: C.blue }}>#{invoiceNumber}</div>
              <div style={{ fontSize: "12px", color: C.inkA(.45), marginTop: "4px" }}>{date}</div>
              {customer && (
                <div style={{ marginTop: "10px", padding: "8px 12px", background: C.blueL, borderRadius: "9px", border: `1px solid ${C.blueBr}` }}>
                  <div style={{ fontSize: "13px", fontWeight: 700, color: C.ink }}>{customer}</div>
                  {phone && <div style={{ fontSize: "11px", color: C.inkA(.5), marginTop: "2px" }}>{phone}</div>}
                  {email && <div style={{ fontSize: "11px", color: C.inkA(.5) }}>{email}</div>}
                </div>
              )}
            </div>
          </div>
            ...
            <thead>
              <tr style={{ background: C.inkA(.025) }}>
                {["#", "Item / Description", "Qty", "Unit Price", "Total"].map((h, i) => (
                  <th key={h} style={{ padding: "10px 12px", textAlign: i >= 2 ? "right" : "left", fontFamily: ff.mono, fontSize: "9px", color: C.inkA(.38), letterSpacing: ".14em", textTransform: "uppercase", borderBottom: `1px solid ${C.inkA(.07)}` }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {items.map((item, i) => (
                <tr key={i} style={{ borderBottom: `1px solid ${C.inkA(.05)}` }}>
                  <td style={{ padding: "11px 12px", fontFamily: ff.mono, fontSize: "11px", color: C.inkA(.35) }}>{i + 1}</td>
                  <td style={{ padding: "11px 12px" }}>
                    <div style={{ fontSize: "13.5px", fontWeight: 600, color: C.ink }}>{item.name}</div>
                    {item.category?.name && <div style={{ fontSize: "11px", color: C.inkA(.4), marginTop: "2px" }}>{item.category.name}</div>}
                  </td>
                  <td style={{ padding: "11px 12px", textAlign: "right", fontFamily: ff.mono, fontSize: "13px", color: C.inkA(.6) }}>{item.qty}</td>
                  <td style={{ padding: "11px 12px", textAlign: "right", fontFamily: ff.mono, fontSize: "13px", color: C.inkA(.6) }}>₹{item.price?.toLocaleString("en-IN")}</td>
                  <td style={{ padding: "11px 12px", textAlign: "right", fontFamily: ff.mono, fontSize: "13.5px", fontWeight: 700, color: C.ink }}>₹{(item.price * item.qty).toLocaleString("en-IN")}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            <div style={{ width: "280px" }}>
              {[
                ["Subtotal", `₹${subtotal.toLocaleString("en-IN")}`, null],
                discountAmt > 0 ? ["Discount Applied", `−₹${discountAmt.toLocaleString("en-IN")}`, C.green] : null,
                ["GST (18%)", `₹${gst.toLocaleString("en-IN")}`, null],
              ].filter(Boolean).map(([k, v, col]) => (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                  <span style={{ fontSize: "13px", color: C.inkA(.5) }}>{k}</span>
                  <span style={{ fontFamily: ff.mono, fontSize: "13px", color: col || C.inkA(.7), fontWeight: col ? 700 : 500 }}>{v}</span>
                </div>
              ))}
              <div style={{ borderTop: `2px solid ${C.inkA(.1)}`, paddingTop: "12px", marginTop: "8px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontFamily: ff.serif, fontSize: "16px", fontWeight: 700, color: C.ink }}>Grand Total</span>
                <span style={{ fontFamily: ff.serif, fontSize: "26px", fontWeight: 900, color: C.blue, letterSpacing: "-.03em" }}>₹{total.toLocaleString("en-IN")}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ marginTop: "22px", display: "flex", gap: "10px", flexWrap: "wrap" }}>
            <div style={{ padding: "8px 14px", borderRadius: "9px", background: C.greenL, border: `1px solid ${C.greenBr}`, display: "flex", alignItems: "center", gap: "7px" }}>
              <Icon d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" size={14} color={C.green} />
              <span style={{ fontFamily: ff.mono, fontSize: "11px", fontWeight: 700, color: C.green }}>PAID · {method}</span>
            </div>
          </div>

          {notes && (
            <div style={{ marginTop: "18px", padding: "12px 14px", borderRadius: "10px", background: C.inkA(.025), border: `1px solid ${C.inkA(.07)}` }}>
              <div style={{ fontSize: "11px", color: C.inkA(.4), fontFamily: ff.mono, marginBottom: "4px", letterSpacing: ".1em" }}>NOTES</div>
              <div style={{ fontSize: "13px", color: C.inkA(.6) }}>{notes}</div>
            </div>
          )}

          <div style={{ marginTop: "28px", textAlign: "center", borderTop: `1px dashed ${C.inkA(.1)}`, paddingTop: "18px" }}>
            <div style={{ fontSize: "11px", color: C.inkA(.3), fontFamily: ff.mono, letterSpacing: ".06em" }}>Thank you for your business!</div>
          </div>
        </div>
      </Card>
    </div>
