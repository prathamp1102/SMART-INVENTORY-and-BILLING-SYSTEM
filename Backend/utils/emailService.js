  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#fff;border-radius:16px;border:1px solid #e5e7eb;overflow:hidden;">
      <div style="background:linear-gradient(135deg,#4f46e5,#7c3aed);padding:28px 32px;text-align:center;">
        <h1 style="margin:0;color:#fff;font-size:20px;font-weight:800;">EVARA</h1>
...
const sendInvoiceEmail = async (to, invoice) => {
  ...
  const html = `
    ...
    <body style="margin:0;padding:0;background:#f0ece4;font-family:Arial,sans-serif;">
      <div style="max-width:560px;margin:36px auto;background:#fff;border-radius:6px;overflow:hidden;">
        ...
        <div style="padding:32px 40px;">
          <p style="font-size:15px;font-weight:700;color:#2d2416;">Hi ${invoice.customerName || "Valued Customer"},</p>
          <p style="font-size:13px;color:#5c4e3a;line-height:1.7;">
            Your invoice <strong>${invoice.invoiceNo || ""}</strong> dated <strong>${issueDate}</strong> for <strong>${grandTotal}</strong> is attached.
          </p>
          <div style="background:#f7f4ef;border-radius:8px;padding:18px 22px;margin:20px 0;">
        ...
        <div style="background:#f7f4ef;padding:16px 40px;font-size:11px;color:#b0a090;text-align:center;">
          Computer-generated email. Do not reply.${orgGst ? " · GST: " + orgGst : ""}
        </div>
      </div>
    </body></html>
  `;
  ...
  const html = `
    ...
    <body style="margin:0;padding:0;background:#f0ece4;font-family:Arial,sans-serif;">
      <div style="max-width:560px;margin:36px auto;background:#fff;border-radius:8px;overflow:hidden;">
        <div style="background:#2d2416;padding:32px 40px;">
          <div style="font-size:22px;font-weight:800;color:#f5f0e8;">${orgName}</div>
          <div style="margin-top:12px;display:inline-block;background:${statusColor};border-radius:20px;padding:4px 14px;">
        ...
        <div style="padding:32px 40px;">
          ...
          <p style="font-size:13px;color:#5c4e3a;line-height:1.7;">
            Your return request <strong>${ret.returnNo}</strong> has been received. Refund: <strong>${refundTotal}</strong>
          </p>
        </div>
        <div style="background:#f7f4ef;padding:16px 40px;font-size:11px;color:#b0a090;text-align:center;">Computer-generated email. Do not reply.</div>
      </div>
    </body></html>
  `;
...
const sendReturnStatusEmail = async (to, ret) => {
  ...
  const html = `
    ...
    <body style="margin:0;padding:0;background:#f0ece4;font-family:Arial,sans-serif;">
      <div style="max-width:560px;margin:36px auto;background:#fff;border-radius:8px;overflow:hidden;">
        <div style="background:#2d2416;padding:32px 40px;">
          <div style="font-size:22px;font-weight:800;color:#f5f0e8;">${orgName}</div>
          <div style="margin-top:12px;display:inline-block;background:${statusColor};border-radius:20px;padding:4px 16px;">
            <span style="font-size:11px;font-weight:700;color:#fff;">${statusLabel.toUpperCase()}</span>
          </div>
        </div>
        <div style="padding:32px 40px;">
          <p style="font-size:15px;font-weight:700;color:#2d2416;">Hi ${ret.customerName || "Valued Customer"},</p>
          <p style="font-size:13px;color:#5c4e3a;line-height:1.7;">
            ${isApproved ? "✅" : "❌"} Your return <strong>${ret.returnNo}</strong> has been
  ...
  await sendEmail({ to, subject: `Return ${ret.returnNo} — ${statusLabel} | ${orgName}`, html });
};
  ...
  const html = `
    ...
    <body style="margin:0;padding:0;background:#f0ece4;font-family:Arial,sans-serif;">
      <div style="max-width:560px;margin:36px auto;background:#fff;border-radius:8px;overflow:hidden;">
        <div style="background:#2d2416;padding:32px 40px;">
          <div style="font-size:22px;font-weight:800;color:#f5f0e8;">${orgName}</div>
          <div style="margin-top:12px;display:inline-block;background:#059669;border-radius:20px;padding:4px 16px;">
            <span style="font-size:11px;font-weight:700;color:#fff;">💸 REFUND SENT</span>
          </div>
        </div>
        <div style="padding:32px 40px;">
          <p style="font-size:15px;font-weight:700;color:#2d2416;">Hi ${ret.customerName || "Valued Customer"},</p>
          <p style="font-size:13px;color:#5c4e3a;">Your refund of <strong style="color:#059669;">${refundTotal}</strong> for return <strong>${ret.returnNo}</strong> has been processed.</p>
          <div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border-radius:10px;padding:20px;text-align:center;margin-top:20px;">
...
<body style="margin:0;padding:0;background:#f0f2f8;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f2f8;padding:32px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(79,70,229,.10);">
        <tr><td style="background:linear-gradient(135deg,#4f46e5,#7c3aed);padding:40px 48px;">
          <p style="margin:0 0 6px;color:rgba(255,255,255,.75);font-size:13px;text-transform:uppercase;">EVARA</p>
          <h1 style="margin:0;color:#fff;font-size:28px;font-weight:800;">Welcome Aboard! 🎉</h1>
        </td></tr>
        <tr><td style="padding:28px 48px;">
          ...
          <p style="color:#444;font-size:14px;line-height:1.8;">Welcome to <strong style="color:#4f46e5;">${orgName || "our organization"}</strong>.</p>
        ...
        <tr><td style="background:#f7f6ff;padding:24px 48px;text-align:center;border-top:1px solid #e8e6ff;">
